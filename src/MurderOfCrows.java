...
public class MurderOfCrows {
	...
	private void findTheDiamond() {
		 ...
		 */
		int deaths = 0;
		for(Crow x : theMurder){
			for(String y: x.getStomachContents()){
				if(y =="diamond"){
					System.out.println(x.getName() + " ate the diamond! " + deaths + " crows' stomachs had to be cut open before it was found");
				}
				else{
					deaths+=1;
				}
